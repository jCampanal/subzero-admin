...
const useStyles = makeStyles((theme) => ({
    item: (props) => ({
        ...
        '&.active': {
            color: theme.palette.text.primary,
            backgroundColor: theme.palette.type === 'light' ? 'rgba(0, 0, 0, .05)!important' : 'rgba(255, 255, 255, .1)!important',
            pointerEvents: 'none',
...
function FuseNavVerticalItem(props) {
    ...
    return useMemo(() => {
        const createIcon = (icon) => {
            switch (icon.slice(0, 3)) {
                case 'fa-':
                    return <i className={clsx(`fuse-list-item-icon pl-2 text-14 flex-shrink-0 fa ${icon}`, item.iconClass)} color="action" />;
                case '':
                    return '';
                default:
                    return (
