...
import {deleteProduct, postProduct, putProduct} from '../../../api-conn/products';
...
const ProductForm = () => {
    ...
    const methods = useForm({
        defaultValues: {
            ...
            salesUnitsId: product.id ? product.salesUnits.map((item) => ({id: item.saleUnitId})) : [],
            decimals: product.id ? product.salesUnits.map((item) => ({id: item.saleUnitId, accept: item.decimals})) : [],
            file: null,
        },
        mode: 'all',
    ...
    const saveData = () => {
        ...
        getValues().salesUnitsId.map((unit) => formData.append('SalesUnitsId', unit.id));
        getValues().decimals.map((decimal) => formData.append('Decimals', decimal.accept));
    ...
    return (
        <FormProvider {...methods}>
            <FusePageCarded
                ...
                header={<FormHeader saveCallback={saveData} deleteCallback={removeProduct} />}
                contentToolbar={<div className="p-16 sm:p-24 max-w-2xl">{id ? <h1>{product.name}</h1> : <h1>{t('CREATE_NEW')}</h1>}</div>}
                content={loading ? <FuseLoading /> : <FormControls imgUrl={product.imageURL} />}
            />
        </FormProvider>
    );
};
