...
import {deleteCategory, postCategory, putCategory} from '../../../api-conn/categories';
...
const CategoryForm = () => {
    ...
    const category = id ? location.state.category : {name: '', link: '', imageURL: ''};
    ...
    useEffect(() => {
        document.title = `${id ? 'Edit category' : 'Create category'} - Subzero Ice Service`;
    }, [id]);
    ...
    return (
        <FormProvider {...methods}>
            <FusePageCarded
                ...
                header={<FormHeader saveCallback={saveData} deleteCallback={removeCategory} />}
                contentToolbar={<div className="p-16 sm:p-24 max-w-2xl">{id ? <h1>{category.name}</h1> : <h1>{t('CREATE_NEW')}</h1>}</div>}
                content={loading ? <FuseLoading /> : <FormControls imgUrl={category.imageURL} />}
            />
        </FormProvider>
    );
};
