...
function Blacklist() {
    ...
    const loadBlacklisted = async () => {
        ...
        if (data) {
            ...
            dispatch(
                showMessage({
                    message: 'We are facing problems to get the blacklisted list. Please try again later',
                    variant: 'error',
                })
            );
        }
    ...
    return (
        <FusePageCarded
            ...
            content={loading ? <FuseLoading /> : <BlacklistTable items={blacklisted} rows={rows} />}
            innerScroll
        />
    );
}
